...
- Front-end flows: `cd cardinal && npm run dev` for the Vite server, `npm run tauri dev -- --features dev` for the desktop shell, `npm run tauri build` for release binaries, and `npm run build` for the static bundle.
...
- For UI and performance validation, follow `TESTING.md`: rebuild with `npm run build`, profile in Chrome DevTools, and capture FPS or memory regressions.
